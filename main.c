...
int main(void)
{
    ...
    TimerA0_Init();
    while(1){
        key_pressed = poll_keys();//wait for a button to be pressed
        delayMs(500);//debounce
        if(key_pressed > 0 & key_pressed <7){//change frequency
            switch(key_pressed){
            case 1 :
        ...
        else if(key_pressed > 6 & key_pressed < 10){//change the waveform
            switch(key_pressed){
            case 7 :
        ...
        else{//change the duty cycle when waveform is a square wave
            switch(key_pressed){
            case 10 :
                //if DC > 10%, decrease DC by 10%
                if(duty_cycle > 10 & waveform == SQUARE_WAVE){
                    duty_cycle -= TEN_PERCENT;
                }
                break;
            case 11 :
                //set DC to 50%
            ...
            case 12 :
                ...
                if(duty_cycle < 90 & waveform == SQUARE_WAVE){
                    duty_cycle += TEN_PERCENT;
                }
                break;
            default :
...
void TA0_0_IRQHandler(void) {
      ...
      switch(waveform){
          case SQUARE_WAVE :
                if (isr_square_count > (100-duty_cycle) & isr_square_count < 100){//set to 2V for appropriate amount of time
          ...
          case SINE_WAVE :
              ...
              Drive_DAC(TempDAC_Value);
              if(isr_sine_count == 239){
                  isr_sine_count = 0;
              }
              isr_sine_count ++;

              TIMER_A0->CCR[0] += 24000000/240/frequency;
              break;
          default :
